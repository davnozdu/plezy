...
bool WaylandVideoSurface::Create(GtkWidget* view, std::string* error) {
  ...
  if (owns_wl_display_ && xdg_wm_base_ != nullptr) {
    ...
    // Commit to apply the role and flush so the compositor receives it.
    // eglCreateWindowSurface may internally roundtrip and needs the
    // surface role to be committed first.
